...
def parse_ics(file_name: str) -> list[dict]: #Gets file and parses through it to extract relevant components to a dictionary and returns a list of a schedule's classes and components
    ...
    with open(file_name, 'rb') as file: #Opens and reads .ics file to parse and extract relevant components; 'rb' or read binary for icalendar format
        ...
        for components in cal.walk():
            if components.name == "VEVENT": #Found a single event/class so set up a dictionary of relevant components and add to events list
                ...
                event["start_time"] = components.get("DTSTART").dt #dt allows for better readability of datetime
                event["end_time"] = components.get("DTEND").dt

                if components.get("RRULE"): #Get which days of the week the class happens and when quarter ends
                    if "BYDAY" in components.get("RRULE"): #Get days in component of RRULE with BYDAY key 
                        event["days"] = components.get("RRULE")["BYDAY"]
                    if "UNTIL" in components.get("RRULE"): #Get the end date of quarter with UNTIL key in RRULE
